...
<?php
$nip = $_SESSION['nip'];
$q_mengajar = mysql_query("SELECT * FROM mengajar LEFT JOIN kelas ON mengajar.id_kelas=kelas.id_kelas LEFT JOIN jurusan ON kelas.id_jurusan=jurusan.id_jurusan LEFT JOIN mapel ON mengajar.id_mapel=mapel.id_mapel LEFT JOIN guru ON mengajar.nip=guru.nip WHERE mengajar.nip=$nip");
if(isset($_POST['go'])) {
...
<div class="content">
  <div class="content-font">
    <center>
      <h3>INPUT NILAI</h3></center>
      <form class="" action="input_nilai.php" method="post">
      ...
      <?php
      if (isset($_POST['go'])) { ?>
        Menampilkan Data <font style="color: blue;"><?php echo $d_temp['nama_kelas'] ?> - <?php echo $d_temp['nama_jurusan'] ?></font> Mata Pelajaran <font style="color: red;"><?php echo $d_temp['nama_mapel'] ?></font> <br><br>
                ...
                <?php if($d_nilai){ ?>
                  <td>
                    <form class="" action="nilai_form.php" method="post">
                      ...
                      <input type="hidden" name="nama_guru" value="<?php echo $d_temp['nama_guru']; ?>">
                      <input type="hidden" name="nama_siswa" value="<?php echo $d['nama_siswa']; ?>">
                      <input type="hidden" name="kelas_jurusan" value="<?php echo $d['nama_kelas']; ?> - <?php echo $d['nama_jurusan'] ?>">
                      <input type="hidden" name="nama_mapel" value="<?php echo $d_temp['nama_mapel']; ?>">

                      <input type="hidden" name="id_nilai" value="<?php echo $d_nilai['id_nilai'] ?>">
